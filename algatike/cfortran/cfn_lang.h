...
#	define CFN_GOTO(LABEL_NAME) \
	goto CFN_PPCAT_2(L, LABEL_NAME)

#	define CFN_LABEL(LABEL_NAME) \
	CFN_PPCAT_2(L, LABEL_NAME):
...
#	define CFN_DO_LABEL_STEP(LABEL_NAME, IDX, START, STOP, STEPS) \
		size_t CFN_PPCAT_7(cfn_jmp_, IDX, START, STOP, STEPS, L, LABEL_NAME) = CFN_MAX(0, CFN_CAST(size_t, ((STOP - START + STEPS) / STEPS))) - 1; \
		CFN_DO_STEP(IDX, START, STOP, STEPS) \
			if (cfn_idx == CFN_PPCAT_7(cfn_jmp_, IDX, START, STOP, STEPS, L, LABEL_NAME)) { \
				CFN_GOTO(LABEL_NAME); \
			}
...
#	define CFN_DO_LABEL(LABEL_NAME, IDX_NAME, START, STOP) \
		CFN_DO_LABEL_STEP(LABEL_NAME, IDX_NAME, START, STOP, 1)

#	define CFN_CONTINUE(LABEL_NAME) } CFN_LABEL(LABEL_NAME) ;
